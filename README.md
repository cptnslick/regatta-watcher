...
`regatta_watcher.py` fetches the page, pulls out just the "Results" section (so unrelated page changes don't trigger false alarms), and hashes it. Each run compares against the hash from last time (`last_hash.txt`). If it changed, it posts to Discord via webhook.
